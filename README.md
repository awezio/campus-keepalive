...
- **系统托盘**: 后台运行，托盘图标实时显示在线、连接中、离线、暂停状态
- **双模式切换**: 支持自动判断、仅保活、自动登录三种模式
- **现代界面**: Windows 版本使用 PySide6 图形界面，耗时网络操作后台执行
...
```yaml
keepalive:
  interval_seconds: 120  # 默认 2 分钟检测一次
```
...
3. 可以使用 `KeepAlive.exe --check` 做一次命令行网络检测，并查看 `logs/keepalive.log`
...
- **语言**: Python 3.8+
- **GUI 框架**: PySide6（Windows 版本）
- **HTTP 请求**: requests
- **加密**: cryptography (Fernet)
- **配置管理**: PyYAML
- **系统托盘/通知**: PySide6 QSystemTrayIcon
...
├── pyside_app.py         # PySide6 主窗口、设置窗口和系统托盘
├── service_controller.py # 非阻塞服务控制器
├── mode_decider.py       # 自动判断运行模式
├── resources.py          # 源码/EXE 资源路径解析
├── heartbeat.py          # 心跳服务，定时发送请求
├── network_checker.py    # 网络状态检测
├── auto_login.py         # 自动登录模块
├── monitor.py            # 超时检测脚本
├── config_manager.py     # 配置管理与加密
├── tray_icon.py          # 旧版 pystray 托盘实现（保留兼容，不作为主入口）
├── notifier.py           # 旧版通知模块（保留兼容，不作为主入口）
├── config_gui.py         # 旧版 Tkinter 配置界面（保留兼容，不作为主入口）
